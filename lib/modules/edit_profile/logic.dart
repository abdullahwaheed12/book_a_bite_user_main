...
class EditProfileLogic extends GetxController {
  ...
  setData(DocumentSnapshot? userModel) {
    nameController = TextEditingController(text: userModel!.get('name'));
    emailController = TextEditingController(text: userModel.get('email'));
    addressController = TextEditingController(text: userModel.get('address'));
    phoneController = TextEditingController(text: userModel.get('phone'));
    Map<String, dynamic> map = userModel.data() as Map<String, dynamic>;
    if (map.containsKey('image')) {
      // Replace field by the field you want to check.
      downloadURL = userModel.get('image');
      if (downloadURL != null) {
        log('URL-->>${downloadURL!.length}---');
      }
    }
  }
